...
$apiKey = $gateway['dogecoin_api_key'];
$pin = $gateway['secret'];
$version = 2; // the API version
$block_io = new BlockIo($apiKey, $pin, $version);
$received = $block_io->get_transactions(array('type' => 'received', 'addresses' => $gateway['dogecoin_address']));
$payment_status="0";
if($received->status == "success") {	
			...
			foreach($dt as $k=>$v) {
				...
				if($time+600 > time()) {
					if($gateway['dogecoin_confirmations'] > $confirmations) {
						if($_SESSION['doge_amount'] == $amount) {
