...
$apiKey = $gateway['litecoin_api_key'];
$pin = $gateway['secret'];
$version = 2; // the API version
$block_io = new BlockIo($apiKey, $pin, $version);
$received = $block_io->get_transactions(array('type' => 'received', 'addresses' => $gateway['litecoin_address']));
$payment_status="0";
if($received->status == "success") {	
			...
			foreach($dt as $k=>$v) {
				...
				if($time+600 > time()) {
					if($gateway['litecoin_confirmations'] > $confirmations) {
						if($_SESSION['ltc_amount'] == $amount) {
